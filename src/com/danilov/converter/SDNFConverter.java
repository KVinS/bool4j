...
public class SDNFConverter implements Converter {
	...
	private Expression toExpression(final List<Map<String, Boolean>> variablesSets, final List<String> keys) {
		...
		Operator res = Operators.getOperator(Operators.DISJUNCTION.REPRESENTATION);
		Operator tmp;
		Operator tmp2;
		for (Map<String, Boolean> set : variablesSets) {
			tmp = Operators.getOperator(Operators.CONJUNCTION.REPRESENTATION);
			for (String key : keys) {
				Expression exp;
			...
			if (!res.firstOperandSet()) {
				...
				tmp2 = res;
				res = Operators.getOperator(Operators.DISJUNCTION.REPRESENTATION);
